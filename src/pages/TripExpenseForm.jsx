...
function TripExpenseForm() {
  ...
  useEffect(() => {
    if (!isEdit) return
    fetch(`${API}/api/trips/${tripId}`, { headers: authHeaders() })
      .then(r => r.json())
      .then(d => {
        const exp = d.expenses.find(e => e.id === parseInt(expenseId))
        if (exp) {
          setTitle(exp.title)
  ...
  const save = () => {
    ...
    const url = isEdit
      ? `${API}/api/trip-expenses/${expenseId}`
    ...
    fetch(url, {
      ...
      body
    }).then(() => navigate(`/travel/trip/${tripId}`))
  }
  ...
  return (
    <div className="page">
      <Link to={`/travel/trip/${tripId}`} className="back-link">← Back</Link>
      <h2 style={{ marginBottom: 16 }}>{isEdit ? '✏️ Edit Expense' : '✈️ Add Expense'}</h2>

      <div className="card">
        <label className="label">Title</label>
        <input className="input" value={title} onChange={e => setTitle(e.target.value)}
          placeholder="e.g. Flight tickets" style={{ marginBottom: 14 }} />
